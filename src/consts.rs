pub const ROUTER_X_MIN_DIFF: f64 = 64.;
pub const ROUTER_Y_MIN_DIFF: f64 = 64.;
pub const ROUTER_TIME_MIN_DIFF: f64 = 64.;
pub const ROUTER_BYPASS_PAD: f64 = 3.;
pub const ROUTER_POPS_LIMIT: Option<usize> = Some(512);
pub const ROUTE_RESET_TICKS_MIN: i32 = 32;
pub const ROUTE_RESET_TICKS_MAX: i32 = 256;
pub const OVERMIND_DECREES_BY_TICK: usize = 5;
// pub const ALLY_SPLIT_DENSITY: f64 = 0.15;
pub const ENEMY_SPLIT_DENSITY: f64 = 0.15;
pub const ENEMY_PREDICT_ROUTE_LEN: f64 = 256.;

// pub const COMPACT_DENSITY: f64 = 0.03;
// pub const ESCAPE_BOUNCE_FACTOR: f64 = 80.0;
// pub const SPLIT_MAX_FORMS: usize = 12;
// pub const HELPER_BY_ESCAPE_DIST_RATIO_SQ: f64 = 4.0 * 4.0;
// pub const SCOUT_RANGE_FACTOR: f64 = 2.0;
// pub const HUNT_RANGE_FACTOR: f64 = 3.0;
// pub const NUKE_ENEMY_CLOSENESS: f64 = 1.1;
// pub const HEAL_REQUEST_LOW_FACTOR: f64 = 0.5;
